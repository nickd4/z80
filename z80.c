...
static INLINE void exec_opcode(z80* const z, uint8_t opcode);
static INLINE void exec_opcode_cb(z80* const z, uint8_t opcode);
static INLINE void exec_opcode_dcb(
    z80* const z, const uint8_t opcode, const uint16_t addr);
static INLINE void exec_opcode_ed(z80* const z, uint8_t opcode);
static INLINE bool exec_opcode_ddfd(z80* const z, uint8_t opcode, uint16_t* const iz);
...
INLINE bool exec_opcode_ddfd(z80* const z, uint8_t opcode, uint16_t* const iz) {
...
INLINE void exec_opcode_cb(z80* const z, uint8_t opcode) {
  z->cyc += 8;
  inc_r(z);
...
INLINE void exec_opcode_dcb(z80* const z, uint8_t opcode, uint16_t addr) {
...
INLINE void exec_opcode_ed(z80* const z, uint8_t opcode) {
  z->cyc += cyc_ed[opcode];
  inc_r(z);
  switch (opcode) {
...
INLINE void exec_opcode(z80* const z, uint8_t opcode) {
  z->cyc += cyc_00[opcode];
  inc_r(z);

loop: // for invalid DD/FD opcodes we return here to process normal opcode
  switch (opcode) {
...
long z80_step(z80* const z, long n) {
  long i;
  for (i = 0; i < n; ++i) {
    if (z->halted) {
      //exec_opcode(z, 0x00);
